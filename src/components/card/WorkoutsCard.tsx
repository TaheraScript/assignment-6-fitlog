        ...
        <div className="p-5 space-y-2">
          <div className="flex gap-2">
            {data.muscleGroups.map((muscle, index) => (
              <div
                key={index}
                className="badge font-inter font-bold uppercase text-[11px] rounded-4xl bg-[#ccff00] text-black "
              >
                {muscle}
              </div>
            ))}
          </div>
          <h2 className="text-[18px] font-oswald uppercase font-bold text-white">
            {data.name}
          </h2>
