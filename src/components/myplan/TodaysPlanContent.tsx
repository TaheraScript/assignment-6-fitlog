...
interface ITodaysPlanContentProps {
  data: IWorkout[];
  onRemove?: (id: number) => void; // added: optional callback to remove an item from the list
  onMarkDone?: (id: number) => void; // added: optional callback for "Mark as Done"
}

const TodaysPlanContent = ({ data, onRemove, onMarkDone }: ITodaysPlanContentProps) => {
  if (data.length === 0) {
    return (
        ...
        >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="btn btn-sm bg-[#ccff00] text-[#0F1115] font-bold font-inter text-[12px] rounded-lg border-none hover:bg-lime-300"
            >
              ✓ Mark as Done
            </button>
            <button
